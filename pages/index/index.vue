<template>
	<view>
		...
		<view class="uni-padding-wrap uni-common-mt">
			...
			<view>蓝牙列表</view>
			<scroll-view class="uni-scroll" scroll-y @touchmove.stop.prevent="moveHandle" @click.stop="moveHandle">
				
			<view v-for="(item, index) in list" :key="index">
				<view class="uni-list_name">蓝牙名称:{{ item.name || item.localName }}</view>
				<!-- <view class="uni-list_item">信号1强度:{{ item.RSSI }}dBm</view> -->
		...
		<view style="height: 100px;">蓝牙</view>
		</view>
		<!-- 遮罩 -->
		...
		async queryDevices() {
			...
			var arry=[]
			 this.list.forEach((item,index)=>{
				if(item.name!=="未知设备"){
					// console.log(777,this.list.length,index)
					arry.push(item)
			 ...
			 this.maskShow = true;
			 this.list=arry;
			 console.log(this.list,"8888",arry)
